...
def plot_centroids(
    ax, centers, color="k", marker="X", s=90, edgecolor="white", linewidth=1.0
):
    ax.scatter(
        centers[:, 0],
        centers[:, 1],
...
# Figure K.1 – Effect of standardization
def fig_k1_scaling_effect():
    # Data with different scales on the two dimensions
    X, y = make_blobs(
        n_samples=600,
        centers=[(-3, 0), (3, 0), (0, 5)],
        cluster_std=[1.0, 2.0, 0.7],
        random_state=RANDOM_STATE,
    )
    # Stretch x-axis to accentuate scale imbalance
    X_unscaled = X.copy()
    X_unscaled[:, 0] *= 8.0
    ...
    ax0 = scatter2d(X_unscaled, y_raw, "Without standardization", ax=axes[0])
    plot_centroids(ax0, km_raw.cluster_centers_)
    ax1 = scatter2d(X_scaled, y_scaled, "With standardization (z-score)", ax=axes[1])
...
# Figure K.2 – Elbow and silhouette vs k
def fig_k2_elbow_silhouette():
    X, _ = make_blobs(
        n_samples=1000, centers=4, cluster_std=1.1, random_state=RANDOM_STATE
    )
...
# Figure K.3 – Sensitivity to initialization
def fig_k3_init_sensitivity():
    ...
    # Two solutions with different seeds, n_init=1 to highlight seed effect
    ...
    # Inertia distribution over many runs (n_init=1)
    runs = 30
    inertias = []
    ...
    scatter2d(X, y_a, "Solution A (seed=10)", ax=axes[0])
    plot_centroids(axes[0], km_a.cluster_centers_)
    scatter2d(X, y_b, "Solution B (seed=999)", ax=axes[1])
    plot_centroids(axes[1], km_b.cluster_centers_)
    axes[2].hist(inertias, bins=8, color="tab:blue", alpha=0.85)
    axes[2].set_title("Inertia distribution (30 runs)")
    axes[2].set_xlabel("Inertia")
...
def fig_k4_outliers_effect():
    ...
    # Compute centroid shift (matching by minimum distance)
    C0 = km_clean.cluster_centers_
    C1 = km_out.cluster_centers_
    # Associate each centroid in C0 with the nearest in C1
    ...
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))
    ax0 = scatter2d(X, km_clean.labels_, "Without outliers", ax=axes[0])
    plot_centroids(ax0, C0)
    ax1 = scatter2d(
        X_with_out,
        km_out.labels_,
        f"With outliers (mean shift={np.mean(shifts):.2f})",
        ax=axes[1],
    )
...
# Figure K.5 – Adverse case: non-spherical shapes (two moons and anisotropic clusters)
def fig_k5_adverse_cases():
    ...
    # Anisotropic clusters: linear transform of blobs
    X_b, _ = make_blobs(
        n_samples=600, centers=3, cluster_std=[1.0, 0.6, 0.8], random_state=RANDOM_STATE
    )
    ...
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))
    scatter2d(X_m, y_m, "Two moons (K-Means struggles)", ax=axes[0])
    plot_centroids(axes[0], km_m.cluster_centers_)
    scatter2d(X_a, y_a, "Anisotropic clusters (non-spherical boundaries)", ax=axes[1])
