...
def scatter2d(X, labels=None, title="", ax=None, s=14, cmap="tab10"):
    ...
    if labels is None:
        ...
        scatter = ax.scatter(
            X[:, 0], X[:, 1], s=s, c=labels, cmap=cmap, alpha=0.9, edgecolor="none"
        )
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title(title)
...
def build_knn_affinity(X, n_neighbors=15, mode="connectivity", include_self=False):
    ...
    G = kneighbors_graph(
        X, n_neighbors=n_neighbors, mode=mode, include_self=include_self, n_jobs=-1
    )
    # Make symmetric
    G = G.maximum(G.T)
    return G
...
def self_tuning_affinity(X, k_local=7):
    # Zelnik-Manor & Perona (2004): sigma_i = distance to k_local-th neighbor
    D = pairwise_distances(X, metric="euclidean")
    # Sort distances per row
    sortD = np.sort(D, axis=1)
    # Index 1.. => 0 is distance to self
    k_idx = min(k_local, sortD.shape[1] - 1)
    sigma = sortD[:, k_idx] + 1e-12
    # Matrix sigma_i * sigma_j
    Sigma = np.outer(sigma, sigma)
    A = np.exp(-(D**2) / Sigma)
    np.fill_diagonal(A, 0.0)
    # Optional: sparsify by keeping only mutual k-NN for stability
    knn = kneighbors_graph(
        X, n_neighbors=k_local, mode="connectivity", include_self=False, n_jobs=-1
    )
...
# Figure 2.1 – Two moons: Original vs K-Means vs Spectral
def fig_2_1():
    ...
    scatter2d(X, None, "Original space", ax=axes[0])
...
# Figure 2.3 – Effect of RBF parameter (gamma)
def fig_2_3():
...
# Figure 2.4 – Eigengap on normalized Laplacian
def fig_2_4():
    ...
    # Build a simple weighted affinity (0/1 connected) or use distances -> minimal local RBF
    # Here we use a simple weighting of 1 for edges (connectivity).
    W = G  # already symmetric
    # Normalized Laplacian
    L = csgraph.laplacian(W, normed=True)
    # Compute smallest eigenvalues (excluding the numerically unstable zero component)
    k_eval = 10
    # eigsh on symmetric sparse matrix
    ...
    if len(gaps) > 0:
        ...
        ax.axvline(
            idx, color="tomato", linestyle="--", alpha=0.7, label=f"eigengap → k≈{idx}"
        )
        ax.legend(loc="best")
    ax.set_xlabel("Eigenvalue index")
    ax.set_ylabel("Eigenvalue (L_norm)")
    ax.set_title("Eigengap for choosing k")
    fig.tight_layout()
    fig.savefig(os.path.join(OUT_DIR, "02_04_eigengap.png"), dpi=160)
    plt.close(fig)


# Figure 2.5 – Spectral embedding: row normalization
def fig_2_5():
    X, _ = make_moons(n_samples=600, noise=0.06, random_state=RANDOM_STATE)
    # Build k-NN affinity (connectivity) and normalized Laplacian
    W = build_knn_affinity(X, n_neighbors=15, mode="connectivity", include_self=False)
    L = csgraph.laplacian(W, normed=True)

    # Take the 2 eigenvectors for the smallest eigenvalues (avoid the constant one if present)
    k = 2
    evals, evecs = eigsh(L, k=k + 1, which="SM", tol=1e-4, maxiter=5000)
    # Drop the first eigenvector (typically associated with eigenvalue ~0)
    order = np.argsort(evals)
    evecs = evecs[:, order]
    U = evecs[:, 1 : k + 1]  # take the two non-trivial components
    ...
    axes[0].set_title("Embedding U (not normalized)")
    axes[0].set_xticks([])
    axes[0].set_yticks([])
    axes[1].scatter(
        U_row[:, 0], U_row[:, 1], c=y_norm, cmap="tab10", s=14, edgecolor="none"
    )
    axes[1].set_title("Embedding U normalized (rows to unit norm)")
    axes[1].set_xticks([])
    axes[1].set_yticks([])
    fig.tight_layout()
...
def fig_2_6_optional():
    ...
    # Spectrum with local scale (self-tuning)
    A_local = self_tuning_affinity(X, k_local=7)
    sp_local = SpectralClustering(
        n_clusters=3,
        affinity="precomputed",
        assign_labels="kmeans",
        random_state=RANDOM_STATE,
    ...
    fig, axes = plt.subplots(1, 2, figsize=(8, 3.6))
    scatter2d(X, y_global, "RBF affinity (global scale)", ax=axes[0])
    scatter2d(X, y_local, "Self-tuning affinity (local scale)", ax=axes[1])
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    fig_2_4()
    fig_2_5()
    # optional (may require more memory/time on some machines)
    fig_2_6_optional()
