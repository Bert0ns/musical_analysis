...
if __name__ == "__main__":
    ...
    plot_clusters_results(filenames, features_reduced, kmeans_labels, RESULTS + "/clusters_plot_kmeans.png")
    plot_tsne_clustering(features_reduced, kmeans_labels, filenames, RESULTS + "/tsne_clusters_plot_kmeans.png")
    ...
    report_km = salva_risultati_markdown(filenames, features_reduced, kmeans_labels, feature_names=None, path=RESULTS + "/report_KM.md", n_repr=5, generi=music_genres)
    report_km_detailed = salva_risultati_markdown(filenames, features_norm_original, kmeans_labels, feature_names=features_names, path=RESULTS + "/report_dettagliato_feature_originali_KM.md", n_repr=5, generi=music_genres)
    ...
    silhouette_score_analysis_kmeans(features_reduced, range_k=(2, 20), fig_name=RESULTS + "/silhouette_analysis_kmeans.png")
    # Elbow method K-Means
    elbow_method_kmeans(features_reduced, range_k=(2, 20), fig_name=RESULTS + "/elbow_analysis_kmeans.png")

    # ============================= DBSCAN CLUSTERING =============================
    print("\nAnalisi esplorativa per DBSCAN (k-distance plot)...")
    try:
        k_distance_plot_dbscan(features_reduced, k=DBSCAN_MIN_SAMPLES, fig_name=RESULTS + "/k_distance_dbscan.png")
    ...
    try:
        silhouette_analysis_dbscan(features_reduced, eps_values, min_samples=DBSCAN_MIN_SAMPLES, metric=DBSCAN_METRIC, fig_name=RESULTS + "/silhouette_analysis_dbscan.png")
    except Exception as e:
    ...
    plot_clusters_results(filenames, features_reduced, dbscan_labels, RESULTS + "/clusters_plot_dbscan.png")
    plot_tsne_clustering(features_reduced, dbscan_labels, filenames, RESULTS + "/tsne_clusters_plot_dbscan.png")
    ...
    report_dbscan = salva_risultati_markdown(filenames, features_reduced, dbscan_labels, feature_names=None, path=RESULTS + "/report_DBSCAN.md", n_repr=5, generi=music_genres)
    report_dbscan_detailed = salva_risultati_markdown(filenames, features_norm_original, dbscan_labels, feature_names=features_names, path=RESULTS + "/report_dettagliato_feature_originali_DBSCAN.md", n_repr=5, generi=music_genres)
    print(f"Report DBSCAN generato: {report_dbscan}")
    print(f"Report dettagliato DBSCAN generato: {report_dbscan_detailed}")

    print("\nPipeline completata (Spectral + K-Means + DBSCAN). Risultati salvati in:", RESULTS)
