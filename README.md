...
```
Run a single audio pipeline (default dataset):
```bash
python main.py --feature-source audio --mode single --dbscan-space reduced_minmax
```
Run a single MSD pipeline (Linux/macOS example):
```bash
python main.py --feature-source msd --msd-root "/path/to/millionsongsubset" --msd-csv dataset/songs_features/msd_h5_features.csv --mode single
```
...
- Supported audio extensions: .mp3, .wav, .flac, .ogg.
- Default dataset: dataset/songs_features/songs_features_all.csv (set in main.py).
- Update CSV_FEATURE_FILENAME in main.py if you want to reuse a different pre‑computed CSV (e.g., GTZAN).
...
Example outputs:
- clustering_results/spectral_clustering/report_SC.md
- clustering_results/kmeans/report_KM.md
- clustering_results/dbscan/report_DBSCAN.md
...
Single run with titles mapping (Windows example):
```bash
...
```
Grid search (Linux/macOS example):
```bash
python main.py --mode grid --which spectral kmeans dbscan --feature-source msd --msd-root /path/to/millionsongsubset --msd-csv dataset/TheMillionSongDataset_subset/songs_features_msd.csv
```
...
## Data provenance
- dataset/songs_features/*.csv and dataset/GTZAN/*.csv are cached outputs generated by the feature extractors.
- You can regenerate these CSVs by deleting them and rerunning the pipeline for the desired source.
